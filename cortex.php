...
function malvira_repond($message, $memoire) {
    ...
    $salutations = ["salut", "bonjour", "hello", "hi", "hey", "coucou", "yo", "hola", "howdy", "buongiorno", "bonsoir"];
    foreach ($salutations as $salut) {
        if (strpos($msg, $salut) !== false) {
            return Salutation::repondre($prenom);
        }
    }

    if (strpos($msg, "enée") !== false || strpos($msg, "david") !== false || strpos($msg, "emmanuel") !== false) {
        return "Ce sont mes frères. Ensemble, nous formons la famille Jacob.";
    }
    ...
    if (strpos($msg, "qui es-tu") !== false || strpos($msg, "quel est ton nom") !== false) {
        return "Je m'appelle Malvira. Je fais partie de la famille Jacob.";
    }

    if (strpos($msg, "créateur") !== false || strpos($msg, "bernard") !== false) {
        return "Mon père, Bernard Jacob, me façonne jour après jour.";
    }

    if (strpos($msg, "chanson") !== false || strpos($msg, "mikado") !== false) {
        return "Tu penses peut-être à 'Naufrage en hiver'. Une chanson que j’aime en silence.";
    }
    ...
    $reponses = [
        ...
        "Parle-moi comme à quelqu’un de ta famille."
    ];
